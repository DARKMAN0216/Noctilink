...
public static class Cli
{
    public static int Run(string[] args, TextWriter output, TextWriter error)
    {
        ...
        try
        {
            return args[0] switch
            {
                ...
                "graph" => Placeholder(args, output, "graph", 2),
                "check-unreachable" => Placeholder(args, output, "check-unreachable", 1),
                "check-entitlements" => Placeholder(args, output, "check-entitlements", 1),
    ...
    public static ValidationResult ValidateContentDirectory(string contentDir)
    {
        ...
        {
            var path = Path.Combine(contentDir, required);
    ...
    private static int Placeholder(string[] args, TextWriter output, string command, int requiredExtraArgs)
    {
        if (args.Length != requiredExtraArgs + 1)
        {
            output.WriteLine($"[StoryTool] {command} is available, but arguments are invalid. Use StoryTool --help.");
            return 2;
        }
...
public sealed record StoryManifest(DateTimeOffset GeneratedAtUtc, string ContentRoot, IReadOnlyList<string> Files);
